...
describe("Domain unit tests (DiceRoll Value Object)", () => {
  ...
  describe("1) Valid DiceRoll ", () => {
    it("1.1) Length = 5 ", () => {
      ...
      expect(diceRoll.getDice()).toHaveLength(5)
    })
    it.each([1, 2, 3, 4, 5])("1.2) Dice %i have a 1-6 value ", (value) => {
      const diceRoll = new DiceRoll()
      const dices = diceRoll.getDice()
    ...
    it("1.3) with dice parameter ", () => {
      ...
      const dices1 = diceRoll1.getDice()
      const diceRoll2 = new DiceRoll(dices1)   
      expect(diceRoll2).not.toBe(diceRoll1)
      expect(diceRoll2.getDice()).not.toBe(dices1)
      expect(diceRoll2.getDice()).toHaveLength(5)
    })
  })
  ...
  describe("2) toggleKeep() ", () => {
    it.each([1, 2, 3, 4, 5])("2.1) Dice %i has isKept toggled", (value) => {
      const diceRoll1 = new DiceRoll()
      const dice1 = diceRoll1.getDice()[value-1]
      const isKept1 = dice1.isKeptDie()

      const diceRoll2 = diceRoll1.toggleKeep(value-1)      
      const dice2 = diceRoll2.getDice()[value-1]
      const isKept2 = dice2.isKeptDie()
  ...
  describe("3) reroll() ", () => {
    ...
    it("3.3) no reroll for dice kept", () => {
      ...
      const dice0Before = diceRoll1Kept.getDice()[0].getValue()

      const diceRoll2 = diceRoll1Kept.reroll([0])         
      const dice0After = diceRoll2.getDice()[0].getValue()
